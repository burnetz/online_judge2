...
public class C {
    public static void main(String[] args) {
        ...
        RMQRAQSegmentTree rmqraq = new RMQRAQSegmentTree(n, 0);
        //高さiまでのマス数の合計を管理する。敢えてリセットは考慮しない。
        int heightSum[] = new int[600_000 + 1];
        Arrays.fill(heightSum, n);
        ...
        for(int i = 0; i < q ; i++) {
            ...
            switch (mode) {
                case 1 -> {
                    int x = sc.nextInt() - 1;
                    int currentLevel = (int)rmqraq.findMin(x, x + 1);
                    rmqraq.add(x, x + 1, 1);
                    //累積和の変更はこの1文だけで済む。
                    heightSum[currentLevel]--;
                }
                case 2 -> {
                    //RMQで最小値を取得できるので本来の1行削除の回数はここで得られる。
                    int resetNum = (int)rmqraq.findMin(0, n);

                    int y = sc.nextInt();

                    System.out.println(n - heightSum[y + resetNum - 1]);
                }
            }
