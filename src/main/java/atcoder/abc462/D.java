...
public class D {
    public static void main(String[] args) {
        ...
        RMQRAQSegmentTree rmqraqSegmentTree = new RMQRAQSegmentTree(1_000_001, 0);

        //RangeAddQueryが使えるなら高速に計算可能。
        //ただし在室時間ではなく犯行開始が可能な時間帯を記録する。
        for(int i = 0; i < n; i++) {
            int start = sc.nextInt();
            int end = sc.nextInt();

            rmqraqSegmentTree.add(start, end - d + 1, 1);
        }

        long count = 0;
        for(int i = 1; i + d <= 1_000_000 ; i++) {
            int min = (int)rmqraqSegmentTree.findMin(i, i + 1);

            //人の組み合わせも聞かれているので計算が必要。
            if(min >= 2) {
                count += (long)min * (min - 1) / 2;
            }
        }
